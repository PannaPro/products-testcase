<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Roles') }}
        </h2>
    </x-slot>

    <div class="py-12 mb-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div>
                    @if( session('alert'))
                        <div>
                            {{ session('alert') }}
                        </div>
                    @endif
                </div>
                <div class="p-6 text-gray-900 text-2xl flex justify-between items-center">
                    
                    <div>                    
                        @foreach ($currentRoleNames as $currentRoleName)
                            <p>{{ 'Name: ' . Auth::user()->name}}</p>
                            <p> {{' Your current role:'. $currentRoleName }}</p>
                        @endforeach
                    </div>
                    
                    
                    
                    <x-primary-button class="ms-6">
                        <a href="{{ route('roles.create') }}">{{ __('Create new role') }}</a>
                    </x-primary-button>
                </div>

                <table class="min-w-full divide-y divide-gray-200 mt-6">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($roles as $role)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap"><a class="text-lg" href="{{ route('roles.edit', $role) }}">{{ $role->name }}</a></td>
                                <td class="px-6 py-4 whitespace-nowrap space-x-2">
                                    <div class="flex items-center space-x-4">
                                        <x-primary-button>
                                            <a href="{{ route('roles.edit', $role) }}">{{ __('Edit') }}</a>
                                        </x-primary-button>
                                        <form action="{{ route('roles.destroy', $role) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="bg-red-500 hover:bg-red-700 text-white font-bold py-1 px-2 rounded-md">Delete</button>
                                        </form>
                                    </div>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap" colspan="4">No role.
                                    <a href="{{ route('roles.create') }}">Create?</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</x-app-layout>



